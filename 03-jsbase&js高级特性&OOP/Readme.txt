...
----------------- 数组  容器(引用数据类型（堆）)  []就是数组 
let arr = [1,2,3,"hello",true,undefiend]
元素：数组中每一个数据叫元素。
索引（下标）：每一个元素对应了一个下标，默认是从0开始。下标就为了方便操作数组中的元素。

创建：let arr = [1,2,3]    new Array()
...
构造器(类)：
    ...
    JS中内置了很多的构造器：
        ...
        Array()  
        Object()
    通过字面量创建的数据，都是new的方式的语法糖。

在JS中，一切都是对象:1  "hello"   true  [1,2,3]   {name:"wangcai"}

数组中常用的方法：
    push 添加一个元素  添加到最后面  返回新数组长度
    unshift  添加一个元素  添加到最前面  返回新数组长度
    pop  删除一个元素  删除最后面元素  返回删除的元素
    shift 删除一个元素  删除最前面元素  返回删除的元素
    join  将数组中的元素使用某个符号连接起来，形成字符串
    reverse 将数组中的元素，倒置
    concat  将多个数组连接成一个数组
    sort 排序
    slice 切割数组
    toString 把数组转换为字符串，并返回结果
-------------------- ES6 中对数组进行扩展 增加很多的新方法 
